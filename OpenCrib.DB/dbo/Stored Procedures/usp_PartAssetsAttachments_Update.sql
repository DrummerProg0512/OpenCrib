...
CREATE PROCEDURE [dbo].[usp_PartAssetsAttachments_Update]
    @PartAssetsAttachmentID INT,
    @AssetID BIGINT = NULL,
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @PartAssetsAttachmentID IS NULL OR @PartAssetsAttachmentID = 0
        BEGIN
            RAISERROR('PartAssetsAttachmentID is required and must be greater than 0.', 16, 1);
            SET @RowsAffected = 0;
            RETURN 1;
        END;
        
        -- Verify record exists
        IF NOT EXISTS (SELECT 1 FROM [dbo].[PartAssetsAttachments] WHERE [PartAssetsAttachmentID] = @PartAssetsAttachmentID)
        BEGIN
            RAISERROR('PartAssetsAttachmentID does not exist.', 16, 1);
            SET @RowsAffected = 0;
            RETURN 1;
        END;
        ...
        UPDATE [dbo].[PartAssetsAttachments]
        SET
        ...
        WHERE
            [PartAssetsAttachmentID] = @PartAssetsAttachmentID;
